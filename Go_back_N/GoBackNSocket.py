...
class GoBackNSocket():
    ...
    def __init__(self, localPort, remotePort, remoteAddress, PLR=0.1, segmentSize=1000, windowSize=1000, timeout=1,
                 p=False):
    ...
    def send(self, msg):
        ...
        while self.currentPackage < numPackages:
            ...
            if GoBackNSocket.currentPackage <= self.windowMax:
                ...
                self.pprint(
                    f"Sent package: {str(p[GoBackNSocket.currentPackage].id)} "f"pkg-data: {d}") if self.p else None
                self.t = time.time()
...
if __name__ == '__main__':
    ...
    serverSocket = GoBackNSocket(serverPort, clientPort, serverAdress, 0.1, 20, windowSize, 1, True)
    clientSocket = GoBackNSocket(clientPort, serverPort, serverAdress, 0.1, 20, windowSize, 1, True)

    st = time.time()

    clientSocket.send(
        ("0" * 6 + "1" * 6 + "2" * 6 + "3" * 6 + "4" * 6 + "5" * 6 + "6" * 6 + "7" * 6 + "8" * 6 + "9" * 6) * 10)
