...
with open("supermarket.txt", "w") as f, open(
    "supermarket_customer.txt", "w"
) as fc, open("supermarket_station.txt", "w") as fs:
    ...
    def startCustomers(shopping_list, name, sT, dT, mT):
        ...
        while t < mT:
            ...
            print("new customer, time: ", t)
            ev = Event(t, customer.run, prio=2)
            EventQueue.push(ev)
            i += 1
            t += dT
    ...
    def main():
        ...
        startCustomers(shopping_list1, "A", 0, 200, 10 * 60 + 1)
        startCustomers(shopping_list2, "B", 1, 60, 4 * 60 + 1)
        evQ.start()
